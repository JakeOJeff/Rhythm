...
function menuengine.new(x, y, font, space)
        ...
        for i = 1, #self.entries do
            if not self.entries[i].disabled then
                ...
                if self.cursor == i and #self.entries[i].text > 0 then
                    ...
                    for _, part in ipairs(particles) do part:draw() end
    ...
    function self:update(dt)
        ...
        for i, part in ipairs(particles) do
            if part then
            part:update(dt)
            if part.size <= 0 then table.remove(particles, i) end
            end
        end
        ...
        if not self.disabled then
            ...
            if not self.mouseDisabled then
                ...
                for i = 1, #self.entries do
                    ...
                    if  not self.entries[i].disabled 
                        and menuengine.mouse_y > self.entries[i].y 
                        and menuengine.mouse_y < self.entries[i].y + self.space and #self.entries[i].text > 0 
                        and menuengine.mouse_x > self.entries[i].x 
                        and menuengine.mouse_x < self.entries[i].x + self.entries[i].font:getWidth(self.entries[i].text .. self.normalSelectedBegin ..self.normalSelectedEnd .. self.symbolSelectedEnd) 
                        then
                        -- ENABLING PARTICLES ON MOUSE HOVER
                        local particle = Particle:new(love.mouse.getX(), love.mouse.getY())
                        particle.size = math.random(0,10)
...
function menuengine.update(dt)
    local i
    for i = 1, #menus do menus[i]:update(dt) end
end
